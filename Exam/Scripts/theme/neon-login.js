...
var reCaught = 0, reCaughtFin = 0;
function onCallBack() {
    reCaught = 1;
    $('#error_message').html('');
    $('#form_login .g-recaptcha iframe').css({
        'border': 'none',
        'border-radius': '5px'
    });
    console.log(reCaught);
}
function onExpired() {
    reCaught = 0;
...
function onCallBackFin() {
    reCaughtFin = 1;
    $('#error_message_fin').html('');
    $('#candidate .g-recaptcha iframe').css({
        'border': 'none',
        'border-radius': '5px'
    });
    console.log(reCaughtFin);
}
function onExpiredFin() {
    reCaughtFin = 0;
    $('#error_message_fin').html('This field is required.');
    $('#candidate .g-recaptcha iframe').css({
        'border': '1px solid red',
        'border-radius': '5px'
    });
...
(function ($, window, undefined) {
    ...
    $(document).ready(function () {
        ...
        neonLogin.$container1 = $("#form_candidate_login");
        ...
        neonLogin.$container.validate({
            ...
            submitHandler: function (ev) {
				...
				*/
                if (parseInt(reCaught) !== 1) {
                    $('#error_message').html('This field is required.');
                    //$('.rc-anchor.rc-anchor-light.rc-anchor-normal').addClass('error-message');
                    $('#form_login .g-recaptcha iframe').css({
                        'border': '1px solid red',
                        'border-radius': '5px'
                    });
                    console.log($('.rc-anchor.rc-anchor-light.rc-anchor-normal').html());
                }
                else {
        ...
        //Login Candidate Form & Validation
        neonLogin.$container1.validate({
            rules: {
                finCode: {
                    required: true
                }
                            ...
                            success: function (response) {
                                ...
                                setTimeout(function () {
                                    ...
                                    if (login_status == 'invalid') {
                                        $(".login-page").removeClass('logging-in');
                                        $('.nav').delay(800).show(0);
                                        neonLogin.resetProgressBar1(true);
                                    }
                                    else
                                        if (login_status == 'success') {
                                            // Redirect to login page
                                            setTimeout(function () {
                                                var redirect_url = '/User/SignIn';
        ...
        if (is_lockscreen) {
            ...
            neonLogin.$ls_thumb1 = neonLogin.$container1.find('.lockscreen-thumb');
            neonLogin.$container.validate({
                rules: {
        ...
        $.extend(neonLogin, {
            ...
            resetProgressBar1: function (display_errors) {
                TweenMax.set(neonLogin.$container1, { css: { opacity: 0 } });

                setTimeout(function () {
                    TweenMax.to(neonLogin.$container1, .6, {
                        css: { opacity: 1 }, onComplete: function () {
                            neonLogin.$container1.attr('style', '');
                        }
                    });
                    ...
                    if (display_errors) {
                        ...
                        // Reset password fields
                        neonLogin.$container1.find('input[type="text"]').val('');
                    }
